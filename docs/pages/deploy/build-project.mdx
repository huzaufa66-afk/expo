...
export default function App() {
  ...
  const whatsappNumber = "2349037205015";

  const sendOrder = () => {
    const message =
      `Barde Delivery 24 Order:%0A` +
      `Name: ${name}%0A` +
      `Phone: ${phone}%0A` +
      `Item: ${item}%0A` +
      `Location: ${location}`;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Barde Delivery 24</Text>
      ...
      <TextInput style={styles.input} placeholder="Item" onChangeText={setItem} />
...
const styles = StyleSheet.create({
  container: { flex: 1, padding: 20, justifyContent: "center", backgroundColor: "#eaffea" },
  title: { fontSize: 24, fontWeight: "bold", textAlign: "center", marginBottom: 20, color: "green" },
